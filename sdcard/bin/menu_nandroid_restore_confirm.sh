...
echo "Restore Options For $1" > "$MENU_FILE"
echo "Go Back:menu:.." >> "$MENU_FILE"
echo "Restore All:shell:nandroid-restore_openrecovery.sh $1 $2 --all" >> "$MENU_FILE"
echo "Restore Selected:shell:nandroid-restore_openrecovery.sh $1 $2" >> "$MENU_FILE"
...
if [ $OPEN_RCVR_BKP -eq 1 ]; then
	cd /sdcard/nandroid/openrecovery/$1
else
	cd /sdcard/nandroid/adbrecovery/$1
fi
