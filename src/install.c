...
void run_shell_script(char *fname, int stdoutToUI) 
{
	int idx=0;
	char *pbuf;
	char *args[ARGC_NUM];
	char myname[MAX_LINE_LENGTH];

	fprintf(stderr, "Running Shell Script: \"%s\"\n", fname);
	strcpy(myname, fname);

	args[idx] = malloc(strlen(SHELL_FILE)+1);
	strcpy(args[idx], SHELL_FILE);
	idx++;

	//yeah space in the name can cause a problem
	pbuf = strtok(myname, " ");
	args[idx] = malloc(strlen(pbuf)+1);
	strcpy(args[idx], pbuf);
	idx++;
	...
	{				
  	...
  	fcntl(script_pipefd[0], F_SETFL, f);
				             ...
				             666)) < 0 ) 
		{
			...
			LOGE("Interactive menu disabled, -1 will be returned.\n");
		}
		else
		{
