...
#define MENU_SCRIPT_RUNNER_FILE	  	"scriptrunner"
...
create_menu(const char *fname, const char *shellcmd) 
{
	...
	if (shellcmd != NULL)
	{
		char shellBuf[MAX_LINE_LENGTH + 256];
		//run the script runner
		sprintf(shellBuf, CUSTOM_MENU_PATH"/"MENU_SCRIPT_RUNNER_FILE" "CUSTOM_MENU_PATH"/%s %s", fname, shellcmd);
		fprintf(stderr, "Running menu script %s.\n", shellcmd);
		run_shell_script(shellBuf, 0);
	}	
...
#else
	run_shell_script("/bin/init_recovery.sh "OPEN_RECOVERY_PHONE_SWITCH, 1);
#endif 
    ...
    switch (chosen_item) {
      ...
      case ITEM_SHELL_SCRIPT:
        ...
        ui_print("%s\n", MENU_ITEMS_TARGET[menu_item]);
				...
				run_shell_script(MENU_ITEMS_TARGET[menu_item], 1);
				create_menu(menu_files[menu_level], menu_scripts[menu_level]);
				free(headers);
				headers = prepend_title(MENU_HEADERS, &title_length);
